// contains logic for generation confirmation page after successful form submission

function generateConfirmationPage(data) {
  // set intial HTML
  const contentElement = document.querySelector("div.main-content-background");
  contentElement.classList.add("confirmation-main-content-background");
  contentElement.innerHTML = `<div class="confirmation-main-content">
        <div class="confirmation-logo-wrapper">
          <svg
            class="confirmation-logo-icon"
            version="1.1"
            id="Capa_1"
		...
		C76.021,32.947,76.543,38.126,73.941,42.635z"
          ...
          <div class="confirmation-title-wrapper">
            <h1 class="confirmation-logo-title">SAVE THE</h1>
            <h1 class="confirmation-logo-title">PUPPIES</h1>
          </div>
        </div>
        <p class="confirm-message">
          Thank you for the donation! Your support allows us to help puppies all throughout the nation.
        </p>
        <div class="donation-info-section">
          <h3 class="donation-info-title">DONATION INFORMATION</h3>
          <div class="donation-info-wrapper">
            <div class="donation-info-row">
              <p class="donation-label">Name:</p>
              <p class="donation-info info-name"></p>
            </div>
            <div class="donation-info-row">
              <p class="donation-label">Address:</p>
              <div class="info-address-wrapper">
                <p class="donation-info info-address-1"></p>
                <p class="donation-info info-address-2"></p>
                <p class="donation-info info-address-3"></p>
                <p class="donation-info info-address-4"></p>
                </div>
            </div>
            <div class="donation-info-row">
              <p class="donation-label">Date:</p>
              <p class="donation-info info-date"></p>
            </div>
            <div class="donation-info-row">
              <p class="donation-label">Donation Amount:</p>
              <p class="donation-info info-amount"></p>
            </div>
            <div class="donation-info-row">
              <p class="donation-label">Confirmation ID:</p>
              <p class="donation-info info-confirmation-id"></p>
            </div>
          </div>
        </div>
      </div>`;

  // get donation info elements
  const nameElement = document.querySelector(".info-name");
  const addressLineOneElement = document.querySelector(".info-address-1");
  const addressLineTwoElement = document.querySelector(".info-address-2");
  const addressLineThreeElement = document.querySelector(".info-address-3");
  const addressLineFourElement = document.querySelector(".info-address-4");
  const dateElement = document.querySelector(".info-date");
  const amountElement = document.querySelector(".info-amount");
  const idElement = document.querySelector(".info-confirmation-id");

  // set info elements with user info
  nameElement.textContent = `${data.firstName} ${data.lastName}`;
  addressLineOneElement.textContent = data.streetAddress;
  data.addressLineTwo.trim() === ""
    ? addressLineTwoElement.parentNode.removeChild(addressLineTwoElement)
    : (addressLineTwoElement.textContent = data.addressLineTwo);
  addressLineThreeElement.textContent = `${data.city}, ${data.state}, ${data.postalCode}`;
  addressLineFourElement.textContent = data.country;
  amountElement.textContent = data.donationAmount;
  ...
  // generate info
  idElement.textContent = generateConfirmationId();
}
...
export { generateConfirmationPage };
